...
## v5.1 (2026-08-11) — 세션 복원력 + 당일 재매수 가드

2026-08-11 KR 세션에서 "0151P0 13,455원 손절 → 몇 분 뒤 13,510원 즉시 재매수"
휩쏘가 실제로 발생했다. 조사 결과 원인은 세션(`job()`) 크래시 후 재시작이었다:
`monitoring_targets`가 메모리에만 있어 재시작되면 `sold_sl` 상태(당일 재매수 금지)가
통째로 사라지고, 재시작된 세션은 그 종목을 "방금 손절한 종목"이 아니라 새 후보로
처음부터 재평가해 소폭 반등만으로도 재매수했다. 같은 근본 원인(세션 크래시에 대한
복원력 부재)이 "US 시장 몇 주째 거래 안 됨" 사고(아래 참조)와도 직결되어 있었다.

1. **종목별 루프 예외 격리** — `job()`의 신규 매수 평가 루프(`for t_obj in tickers`)에
   try/except가 전혀 없어, 종목 하나의 API 응답 파싱 오류(KeyError 등)만으로도 세션
   전체가 죽었다. 이제 종목 단위로 예외를 잡아 알림 후 다음 종목으로 계속 진행한다.
   `kis_domestic.py`/`kis_api.py`의 `get_current_price`/`get_daily_ohlc` 파싱부에도
   동일하게 방어적 try/except를 추가했다.
2. **세션 크래시 시 당일 재시도** — 과거에는 KR(09:00~09:05)/US(23:30~23:35) 5분
   트리거 창에서 `job()`이 크래시하면 그날은 더 이상 재시도되지 않았다
   (`run_with_recovery`). 이제 크래시 시 백오프 후 같은 세션(당일 장중) 안에서
   `max_consecutive_errors`(5회)까지 재시도하고, 그마저 소진되면 "오늘은 포기"를
   명확히 알림으로 남긴다.
3. **당일 손절 종목 재매수 금지 가드** — `sold_sl`/`sold_tp` 등 실제 청산 기록은
   `trade_journal.json`(디스크)에도 남는다는 점을 활용해, 세션 재시작으로 메모리
   상태가 사라져도 "오늘 이미 손절/청산한 종목"은 `trade_journal`을 조회해 당일
   재매수를 차단한다 (`STOPPED_OUT_TODAY` 가드, aggressive_dca Step 1 게이트 (0)).
4. **레거시 VBO 브레이크아웃 매수 경로 차단** — watch loop 안에 day/swing 전용
   VBO 브레이크아웃 매수 코드가 남아있었는데, `STRATEGY_MODE` 체크가 없어
   aggressive_dca에서 게이트 차단(`blocked`)되거나 자금 부족(`dca_wait`)으로 대기 중인
   종목이 이 경로로 새어 들어가면 추세/AI/섹터/상관관계 게이트를 전부 우회해 매수될
   수 있었다. `STRATEGY_MODE in ('dca', 'aggressive_dca')`일 때 이 경로 전체를 차단.
